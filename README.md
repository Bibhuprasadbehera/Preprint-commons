...
Core Requirement
You want to add author search functionality to your existing research paper database application, similar to how paper search currently works.

Specific Requirements
1. Backend Changes
Create a separate API endpoint for author search (not mixed with paper search)
Search should be done only on the submission_contact field (not the all_authors JSON field)
The endpoint should be at root level (like /authors/search, not /api/papers/authors/search)
Should work with actual database data, not mock data
Should return authors with their paper count and total citations
2. Frontend UI Structure
Add a new "Search Authors" tab in the existing Explore page (alongside "Search Papers", "Interactive Map", etc.)
The author search should have its own search interface similar to the paper search
When you search for an author, you get a list of authors (not papers)
Each author in the list should show:
Author name (from submission_contact)
Number of papers they have
Total citations across all their papers
3. Author Details Functionality
When you click on an author from the search results, it should take you to an Author Details page
This page should show:
Author name
Author statistics (total papers, total citations)
List of ALL papers by that author (similar to how paper details page works)
Each paper should be clickable to go to individual paper details
4. Navigation Flow
The flow should be:

Go to Explore page → Click "Search Authors" tab
Search for author name → Get list of matching authors
Click on specific author → Go to Author Details page
See all papers by that author → Click on specific paper → Go to Paper Details page
5. Technical Requirements
Reuse existing components as much as possible (SearchBar, Card, Pagination, etc.)
Don't break existing functionality (paper search should continue working)
Use actual database data (no mock data)
Should handle pagination for author search results
Should be efficient and not create unnecessary new files
6. Current Issues You're Facing
The API endpoints are returning 404 errors
Even when you try to search, you get "No authors found"
The backend routes are not being properly registered
The paper search functionality got broken in the process
The goal is to have a complete author-centric search and browsing experience that complements the existing paper-centric functionality, allowing users to discover research through authors rather than just paper titles/keywords.